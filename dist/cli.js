          ...
          diagnostics: [
            {
              code: "CLI_USAGE_ERROR",
              severity: "error",
              message: parsed.command ? `Unknown command: ${parsed.command}` : "No command specified. Use: validate | index | query | resolve"
            }
          ]
        },
        2
...
var isDirectRun = process.argv[1] && (process.argv[1].endsWith("/cli.js") || process.argv[1].endsWith("/cli.mjs") || process.argv[1].endsWith("\\cli.js") || process.argv[1].endsWith("\\cli.mjs"));
if (isDirectRun) {
  run(process.argv);
}
export {
